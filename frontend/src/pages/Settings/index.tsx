...
import { User, Key, Sliders, BookOpen, Bot, Save, RotateCcw, Eye, EyeOff, Check, ChevronRight, Sparkles, GraduationCap, MessageSquare, Plus, Trash2, Edit2, X, Globe, Zap, Heart, Database } from 'lucide-react';
                                    ...
                                    <div className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-md rounded-3xl p-6 shadow-sm border border-slate-200 dark:border-slate-700 space-y-8">
                                        ...
                                        <div className="space-y-4">
                                            <h4 className="text-sm font-bold text-slate-800 dark:text-white flex items-center gap-2">
                                                ...
                                                引擎大盘与预载核心 (Model Downloader)
                                            </h4>

                                            <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                                                {models.map(m => {
                                                    const isDownloading = activeDownloads[m.id] !== undefined;
                                                    const progress = isDownloading ? activeDownloads[m.id] : m.progress;

                                                    return (
                                                        <div key={m.id} className="p-4 rounded-xl border border-slate-200 dark:border-slate-700 bg-white/50 dark:bg-slate-900/50 shadow-sm transition-all hover:bg-white dark:hover:bg-slate-800 hover:shadow-md flex flex-col gap-3">
                                                            <div className="flex justify-between items-start">
                                                                <div>
                                                                    <div className="font-bold text-slate-800 dark:text-white">{m.name}</div>
                                                                    <div className="text-xs font-mono text-slate-500 mt-0.5 max-w-[150px] truncate" title={m.id}>{m.id}</div>
                                                                </div>
                                                                <div className="flex flex-col items-end gap-2 shrink-0">
                                                                    {m.status === 'completed' ? (
                                                                        <span className="px-2 py-1 rounded bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400 text-[10px] font-bold">已就绪 (Installed)</span>
                                                                    ) : isDownloading ? (
                                                                        <div className="text-right">
                                                                            <span className="text-[10px] font-bold text-blue-500 animate-pulse">Downloading... {progress}%</span>
                                                                        </div>
                                                                    ) : (
                                                                        <button
                                                                            onClick={() => startDownload(m.id)}
                                                                            className="px-3 py-1.5 rounded-lg bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-900/30 dark:hover:bg-indigo-900/50 text-indigo-600 dark:text-indigo-400 text-xs font-bold transition-colors"
                                                                        >
                                                                            点击获取 ({m.size_estimate})
                                                                        </button>
                                                                    )}
                                                                </div>
                                                            </div>
                                                            <div className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed border-l-2 border-indigo-200 dark:border-indigo-800 pl-2">
                                                                {m.description}
                                                            </div>
                                                            <div className="flex flex-wrap gap-1.5 mt-auto">
                                                                {m.tags.map((t: string) => <span key={t} className="px-1.5 py-0.5 rounded text-[10px] bg-slate-100 dark:bg-slate-800 text-slate-500 font-medium border border-slate-200/50 dark:border-slate-700/50">{t}</span>)}
                                                            </div>
                                                            {isDownloading && (
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="flex gap-3">
                                                <select
                                                    ...
                                                    className="flex-1 px-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl font-mono text-sm focus:ring-2 focus:ring-indigo-500 outline-none cursor-pointer"
                                                >
                                                    {models.filter(m => m.status === 'completed').length > 0 ? (
                                                        ...
                                                        <option value={settings.ragEmbedModel}>{settings.ragEmbedModel} (当前绑定)</option>
                                                    )}
                                                </select>
                                                <button
                                                    onClick={async () => {
